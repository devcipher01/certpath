...
async function handleVerifyApi(request: Request): Promise<Response | null> {
  ...
  // Accept either ?code=CERTIFYPATH-XXXX or ?url=https://certifypath.online/certificate/...
      ...
      try {
        ...
        if (!code) {
          return apiJson(
            {
              valid: false,
              error: "The provided URL does not contain a certificate code. Make sure you're using the full certificate URL (e.g. /certificate/course/name?code=CERTIFYPATH-XXXX).",
            },
            400,
          );
        }
    ...
    return apiJson(
      {
        ...
        usage: [
          "By code: GET /api/verify?code=CERTIFYPATH-XXXX-XXXX",
          "By URL:  GET /api/verify?url=https://certifypath.online/certificate/course/name?code=CERTIFYPATH-XXXX-XXXX",
        ],
      },
      400,
    );
