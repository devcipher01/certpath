...
export const Route = createFileRoute("/courses/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Course not found — CertifyPath" }, { name: "robots", content: "noindex" }] };
    }
    const c = loaderData.course;
    return {
      meta: [
        { title: `${c.title} — Course & Certification | CertifyPath` },
        { name: "description", content: c.tagline },
        { property: "og:title", content: `${c.title} — CertifyPath` },
          ...
          <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 md:grid-cols-[1.1fr_1fr] md:items-center">
            <div>
              ...
              <p className="mt-2 text-muted-foreground">
                Every learner receives an official <span className="font-medium text-foreground">CertifyPath</span> PDF
