...
export const Route = createFileRoute("/certifications/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) return { meta: [{ title: "Certification not found — CertifyPath" }, { name: "robots", content: "noindex" }] };
    const c = loaderData.course;
    return {
      meta: [
        { title: `${c.title} Certification — CertifyPath` },
        { name: "description", content: `Earn the ${c.title} certification for $${c.certPrice} — exam or attestation route.` },
        { property: "og:title", content: `${c.title} Certification — CertifyPath` },
