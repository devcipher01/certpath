...
export const Route = createFileRoute("/api-docs")({
  head: () => ({
    meta: [
      { title: "Certificate Verification API — CertifyPath" },
      { name: "description", content: "Programmatically verify CertifyPath certificates from any platform." },
    ],
  }),
...
const FALLBACK_BASE = "https://certifypath.online";
...
function buildSnippets(base: string) {
  const certUrl = `${base}/certificate/medical-transcriptionist/jane-doe?code=CERTIFYPATH-A1B2-C3D4`;

  const curlExample = `# Option 1 — by certificate code
curl "${base}/api/verify?code=CERTIFYPATH-A1B2-C3D4"
...
async function verifyCertifyPath({ code, url }) {
  const params = code
...
// Usage examples:
verifyCertifyPath({ code: "CERTIFYPATH-A1B2-C3D4" });
verifyCertifyPath({ url: "${certUrl}" });`;

  const pythonExample = `import requests
from datetime import datetime

def verify_certifypath(*, code: str = "", url: str = "") -> dict:
...
# Usage examples:
verify_certifypath(code="CERTIFYPATH-A1B2-C3D4")
verify_certifypath(url="${certUrl}")`;

  const phpExample = `<?php
/**
 * Verify a CertifyPath certificate by code or by profile URL.
 * Pass exactly one of: $code or $profileUrl
 */
function verifyCertifyPath(string $code = "", string $profileUrl = ""): array {
    $param = $code
...
<script>
  (async () => {
    ...
    const el   = document.getElementById("certifypath-badge");
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-16">
        ...
        <div className="mb-10">
          ...
          <p className="mt-3 text-muted-foreground">
            Verify any CertifyPath certificate from your own platform, application, or website.
            Every certificate issued through CertifyPath is instantly queryable — no signup, no API key.
          </p>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                <tr>
                  ...
                  <td className="px-4 py-3 text-muted-foreground">The validation code printed on the certificate (e.g. <code>CERTIFYPATH-A1B2-C3D4</code>)</td>
                </tr>
        ...
        <section className="mb-10 rounded-xl border border-border bg-card p-5 text-sm text-muted-foreground space-y-2">
          ...
          <ul className="ml-1 list-disc space-y-1 pl-4">
            ...
            <li><strong>Two ways to verify:</strong> pass <code>?code=CERTIFYPATH-XXXX</code> or the full <code>?url={base}/certificate/…</code> — the code is extracted from the URL automatically.</li>
            <li><strong>Codes are case-insensitive:</strong> <code>certifypath-a1b2-c3d4</code> and <code>CERTIFYPATH-A1B2-C3D4</code> both work.</li>
          </ul>
        </section>
