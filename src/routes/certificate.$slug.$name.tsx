...
export const Route = createFileRoute("/certificate/$slug/$name")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Certificate — CertifyPath" }, { name: "robots", content: "noindex" }] };
    }
    return {
      meta: [
        { title: `Certificate — ${loaderData.course.title} | CertifyPath` },
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-12">
        <div className="mb-6 flex items-center gap-2 rounded-lg border border-emerald-500/40 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-400">
          ...
          <span>
            This certificate is <strong>verified</strong> — issued by CertifyPath for {result.courseTitle}.
          </span>
        </div>
